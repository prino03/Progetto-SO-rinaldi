...
int get_level(buddyalloc* buddy , int size){
    ...
    //when memory is enough we stop
    while(bucket_size < size){                          
        bucket_size = bucket_size << 1;                 
        pow++;
    }
    return buddy->max_levels - pow - 1;                
}

//returns value of idx bit in the bitmap
int bitmap_ret_bit_value(bitmap* buddy_map, int idx){
    //this calculates the offset of idx in the byte it is contained in
    //then we create a mask to read only the bit we need, doing an and bit a bit 
...
//updates value of a bit of index idx
void bitmap_set_bit(bitmap* buddy_map , int idx, int value){
    ...
    //we have to save all the other values 
    //we create a mask for this purpose
    if(value)buddy_map->map[byte_map_idx] |= mask; 
    
    else buddy_map->map[byte_map_idx] &= ~mask;
}
...
void* alloc_buddy(buddyalloc* buddy , size_t size){
    ...
    //we calculate the offset from the starting point of our memory
...
void free_buddy(buddyalloc* buddy , void* to_free){
    ...
    //we free the childrens 
    bitmap_change_children( buddy->bitmap , block_idx , 0 );
